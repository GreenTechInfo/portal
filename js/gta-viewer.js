...
export class GTACharacterViewer {
    ...
    async loadModel(dffPath, txdPath, ifpPath, modelData = null) {
		...
		try {
			...
			try {
				const pathParts = txdPath.split('/');
				pathParts[pathParts.length - 1] = 'vehicle.txd';
				const vehicleTxdPath = pathParts.join('/');

				if (vehicleTxdPath !== txdPath) {
					await this.loadTXD(vehicleTxdPath);
				}
			} catch (error) {
			}
